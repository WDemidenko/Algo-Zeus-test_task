...
symbols = ["ADAUSDT", "DOGEUSDT", "TRXUSDT", "MATICUSDT", "SOLUSDT", "LTCUSDT", "DOTUSDT", "BUSDDAI", "AVAXUSDT",
           "SHIBUSDT"]
...
def candlestick():
    ...
    candlestick_df = go.Figure(data=[go.Candlestick(x=df['Open time'],
                                                 open=df['Open'],
                                                 high=df['High'],
                                                 low=df['Low'],
                                                 close=df['Close'])])
    ...
    return render_template('candlestick.html', candlestick=candlestick_json, symbol=symbol, interval=interval)


@app.route('/piechart')
def piechart():
    market_caps = get_market_caps(symbols)
    df_market_caps = pd.DataFrame({'Symbol': symbols, 'Market Cap': market_caps})
